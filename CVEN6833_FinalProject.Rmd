...
gagenames <- c("CR_Glen", "Gunn_Grand", "Green_FontRes", "LSR_Lily", "SJR_Bluff", "CR_LeesFerry")
...
#####*Fit a Random Forest Model and predict for Spring flow*
```{r RF Model}
...
```{r Predict with RF}
# Predict stream flow at each gage with the RF models
gage_preds <- list()

for (i in 1:ngages){
  gage_preds[[i]] <- flow_pcaresult$x[,i] * sum(sapply(RF_mods[1:4], function(mod) sum(mod$preds)))
}

names(gage_preds) <- gagenames
...
predvals <- as.data.frame(matrix(numeric(0), nrow = nrow(actvals), ncol = ngages))

for (i in 1:ngages){
  predvals[,i] <- gage_preds[[i]]
}
# 
...
# for (i in 1:ngages){
#   mod_evals[[i]] <- modeval(ann_spring_flow_sel[,i], gage_preds[[i]])
# }

# Boxplot the Correlation and RMSE for each gage
# for (i in 1:ngages){
#   evalplots <- mod_evals[[i]]$corrplot + mod_evals[[i]]$rmseplot + 
#     plot_annotation(title = paste("Correlation and RMSE of the RF Predictions for", gagenames[i]))
#   print(evalplots)
# }
...
for (i in 1:ngages) {
  ...
  predvact_plots[[i]] <- ggplot(plot_data, aes(x = Actual, y = Predicted)) +
    ...
    labs(title = paste("Predicted v. Actual -", gagenames[i]), x = "Actual", y = "Predicted") +
