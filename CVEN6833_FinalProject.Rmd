...
for(i in 1:nnodes){
  ...
  image.plot(lon, lat, zmat, ylim = range(-16,16), xlim = sstrange, main = paste("Node", i, sep = ""))
  contour(lon, lat, (zmat), ylim = range(-16,16), add = TRUE, nlev = 6, lwd = 1)
  map("world2",add=T)}

mtext("Annual Average winter SST - SOM Maps 4x4", outer = TRUE, side = 3, cex = 1.2, line = 1)

# # Plot the composite node data on a map
# nx = 50
# 
# par(oma = c(0,2,3,6))
# par(mfrow = c(n, n))
...
# 
# for(i in 1:nnodes){
#   quilt.plot(xygrid1[,2], xygrid1[,1], nodes[,i], main = paste("Node ", i, sep = ""), zlim = noderange, add.legend = FALSE, 
#              nx = nx, ny = nx)
#   map("world2", add = T)}
# 
# mtext("Winter Annual Average SST - SOM Maps 4x4", outer = TRUE, side = 3, cex = 1.2, line = 1)
# par(oma = c(1,0,0,1))
# par(mar = c(0, 0, 0, 0))
# image.plot(zlim = noderange, legend.only = TRUE, legend.shrink = 1, legend.width = 3)
# 
# library(kohonen)
